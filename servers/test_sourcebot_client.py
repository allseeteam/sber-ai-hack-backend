        ...
        if repos["List"]["Repos"]:  # Try original case first
            for repo_info in repos["List"]["Repos"]:
                ...
                if repo["RawConfig"]:
                    github_stats = {
                        k: v
                        for k, v in repo["RawConfig"].items()
                ...
                print(
                    f"Content size: {repo_info['Stats']['ContentBytes'] / (1024*1024):.1f} MB"
                )
            ...
            print(
                f"Total content size: {stats['ContentBytes'] / (1024*1024*1024):.1f} GB\n"
            )
        else:
...
async def test_search(client: SourcebotClient) -> None:
    ...
    try:
        if search_results["Result"]["Files"]:
            print(
                f"Found {len(search_results['Result']['Files'])} files containing '{search_query}':"
            )
            for file in search_results["Result"]["Files"]:
                ...
                for match in file["ChunkMatches"]:
                    ...
                    print(
                        f"  Location: Line {match['ContentStart']['LineNumber']}, Column {match['ContentStart']['Column']}"
                    )
                    print("  ---")
            ...
            for file in search_results["Result"]["Files"][:2]:
                ...
                source = await client.get_file_source(
                    file_name=file["FileName"], repository=file["Repository"]
                )
